...
import com.github.atris.thunderbolt.Arc;
import com.github.atris.thunderbolt.Fst;
import com.github.atris.thunderbolt.MutableFst;
import com.github.atris.thunderbolt.MutableState;
import com.github.atris.thunderbolt.State;
import com.github.atris.thunderbolt.semiring.Semiring;
import com.github.atris.thunderbolt.semiring.UnionSemiring;
import com.github.atris.thunderbolt.semiring.GallicSemiring;
import com.github.atris.thunderbolt.semiring.GallicSemiring.GallicMode;
import com.github.atris.thunderbolt.semiring.GallicSemiring.GallicWeight;
...
import static com.github.atris.thunderbolt.semiring.GallicSemiring.SHORTLEX_ORDERING;
...
public class Determinize {
  ...
  public MutableFst compute(final Fst fst) {
    ...
    while (!workQueue.isEmpty()) {
      ...
      for (DetArcWork arcWork : arcWorks) {
        ...
        UnionSemiring.UnionWeight<GallicWeight> unionWeight = arcWork.computedDivisor;
  ...
  private static UnionSemiring<GallicWeight, GallicSemiring> makeUnionRing(Semiring semiring,
    ...
    switch (mode) {
      ...
      case NON_FUNCTIONAL:
        ...
        return UnionSemiring.makeForOrdering(gallicSemiring, SHORTLEX_ORDERING, doMerge, UnionSemiring.UnionMode.NORMAL);
      default:
  ...
  private Collection<DetArcWork> groupByInputLabel(final DetStateTuple detStateTuple) {
    ...
    for (DetElement detElement : detStateTuple.getElements()) {
      ...
      for (Arc inputArc : inputState.getArcs()) {
        ...
        UnionSemiring.UnionWeight<GallicWeight> inputArcAsUnion = UnionSemiring.UnionWeight.createSingle(
  ...
  private void normalizeArcWork(final DetArcWork arcWork) {
    ...
    for (int i = 0; i < arcWork.pendingElements.size(); i++) {
      ...
      if (deduped.isEmpty() || deduped.get(deduped.size() - 1).inputStateId != currentElement.inputStateId) {
        ...
        UnionSemiring.UnionWeight<GallicWeight> merged = unionSemiring.plus(lastElement.residual, currentElement.residual);
    ...
    for (int i = 0; i < arcWork.pendingElements.size(); i++) {
      DetElement currentElement = arcWork.pendingElements.get(i);
      UnionSemiring.UnionWeight<GallicWeight> divided = unionSemiring.divide(currentElement.residual, arcWork.computedDivisor);
  ...
  private double computeFinalWeight(final int outputStateId,
    ...
    UnionSemiring.UnionWeight<GallicWeight> result = this.unionSemiring.zero();
    for (DetElement detElement : targetTuple.getElements()) {
      ...
      UnionSemiring.UnionWeight<GallicWeight> origFinal = UnionSemiring.UnionWeight.createSingle(GallicWeight.createEmptyLabels(
  ...
  private static class DetArcWork {
    ...
    UnionSemiring.UnionWeight<GallicWeight> computedDivisor;

    DetArcWork(final int inputLabel, UnionSemiring.UnionWeight<GallicWeight> zero) {
  ...
  static class DetElement implements Comparable<DetElement> {
    ...
    private final UnionSemiring.UnionWeight<GallicWeight> residual;
    ...
    DetElement(final int inputStateId, final UnionSemiring.UnionWeight<GallicWeight> residual) {
    ...
    UnionSemiring.UnionWeight<GallicWeight> getResidual() {
      return residual;
    }

    // returns a copy of this DetElement but with the given weight as the residual weight
    DetElement withResidual(UnionSemiring.UnionWeight<GallicWeight> weight) {
