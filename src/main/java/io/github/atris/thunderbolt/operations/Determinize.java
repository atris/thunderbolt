...
import io.github.atris.thunderbolt.semiring.UnionSemiring;
import io.github.atris.thunderbolt.semiring.GallicSemiring;
...
public class Determinize {
  ...
  private final GallicSemiring.GallicMode gallicMode;

  // runtime state
  private Semiring semiring;
  private Fst inputFst;
  private UnionSemiring<GallicSemiring.GallicWeight, GallicSemiring> unionSemiring;
  ...
  public Determinize(DeterminizeMode mode) {
    ...
    this.gallicMode = (mode == DeterminizeMode.DISAMBIGUATE ?
      GallicSemiring.GallicMode.MIN_GALLIC :
      GallicSemiring.GallicMode.RESTRICT_GALLIC);
  }
  ...
  public MutableFst compute(final Fst fst) {
    ...
    DetElement initialElement = new DetElement(fst.getStartState().getId(),
      GallicSemiring.GallicWeight.createEmptyLabels(semiring.one()));
    ...
    while (!workQueue.isEmpty()) {
      ...
      for (DetArcWork arcWork : arcWorks) {
        ...
        UnionSemiring.UnionWeight<GallicSemiring.GallicWeight> unionWeight = arcWork.computedDivisor;
        for (GallicSemiring.GallicWeight gallicWeight : unionWeight.getWeights()) {
  ...
  private static UnionSemiring<GallicSemiring.GallicWeight, GallicSemiring> makeUnionRing(Semiring semiring,
    ...
    switch (mode) {
      ...
      case DISAMBIGUATE:
        ...
        return UnionSemiring.makeForOrdering(gallicSemiring, Ordering.allEqual(), gallicSemiring::plus,
          UnionSemiring.UnionMode.RESTRICTED);

      case NON_FUNCTIONAL:
        // in this case we do use the real union semantics and want to merge the same label weights
        UnionSemiring.MergeStrategy<GallicSemiring.GallicWeight> doMerge = (a, b) -> {
          Preconditions.checkArgument(a.getLabels().equals(b.getLabels()), "cant merge different labels");
          return GallicSemiring.GallicWeight.create(a.getLabels(), semiring.plus(a.getWeight(), b.getWeight()));
        };
        return UnionSemiring.makeForOrdering(gallicSemiring, GallicSemiring.SHORTLEX_ORDERING, doMerge, UnionSemiring.UnionMode.NORMAL);
      default:
  ...
  private Collection<DetArcWork> groupByInputLabel(final DetStateTuple detStateTuple) {
    ...
    for (DetElement detElement : detStateTuple.getElements()) {
      ...
      for (Arc inputArc : inputState.getArcs()) {
        ...
        UnionSemiring.UnionWeight<GallicSemiring.GallicWeight> inputArcAsUnion = UnionSemiring.UnionWeight.createSingle(
          GallicSemiring.GallicWeight.createSingleLabel(inputArc.getOlabel(), inputArc.getWeight())
        );
  ...
  private void normalizeArcWork(final DetArcWork arcWork) {
    ...
    for (int i = 0; i < arcWork.pendingElements.size(); i++) {
      ...
      if (deduped.isEmpty() || deduped.get(deduped.size() - 1).inputStateId != currentElement.inputStateId) {
        ...
        UnionSemiring.UnionWeight<GallicSemiring.GallicWeight> merged = unionSemiring.plus(lastElement.residual, currentElement.residual);
    ...
    for (int i = 0; i < arcWork.pendingElements.size(); i++) {
      DetElement currentElement = arcWork.pendingElements.get(i);
      UnionSemiring.UnionWeight<GallicSemiring.GallicWeight> divided = unionSemiring.divide(currentElement.residual, arcWork.computedDivisor);
  ...
  private double computeFinalWeight(final int outputStateId,
    ...
    UnionSemiring.UnionWeight<GallicSemiring.GallicWeight> result = this.unionSemiring.zero();
    for (DetElement detElement : targetTuple.getElements()) {
      ...
      UnionSemiring.UnionWeight<GallicSemiring.GallicWeight> origFinal = UnionSemiring.UnionWeight.createSingle(
          GallicSemiring.GallicWeight.createEmptyLabels(
  ...
  private void expandDeferredFinalStates(Deque<DetElement> finalQueue) {
    HashBiMap<Integer, GallicSemiring.GallicWeight> outputStateIdToFinalSuffix = HashBiMap.create();
    while (!finalQueue.isEmpty()) {
      DetElement element = finalQueue.removeFirst();
      for (GallicSemiring.GallicWeight gallicWeight : element.residual.getWeights()) {
        // factorization is like a simple version of the divisor/divide calculation earlier
        Pair<GallicSemiring.GallicWeight, GallicSemiring.GallicWeight> factorized = gallicSemiring.factorize(gallicWeight);
        GallicSemiring.GallicWeight prefix = factorized.getLeft();
        GallicSemiring.GallicWeight suffix = factorized.getRight();
  ...
  private static class DetArcWork {
    ...
    UnionSemiring.UnionWeight<GallicSemiring.GallicWeight> computedDivisor;

    DetArcWork(final int inputLabel, UnionSemiring.UnionWeight<GallicSemiring.GallicWeight> zero) {
  ...
  static class DetElement implements Comparable<DetElement> {
    ...
    private final UnionSemiring.UnionWeight<GallicSemiring.GallicWeight> residual;

    DetElement(final int inputStateId, final GallicSemiring.GallicWeight singleGallic) {
      this(inputStateId, UnionSemiring.UnionWeight.createSingle(singleGallic));
    }

    DetElement(final int inputStateId, final UnionSemiring.UnionWeight<GallicSemiring.GallicWeight> residual) {
    ...
    UnionSemiring.UnionWeight<GallicSemiring.GallicWeight> getResidual() {
      return residual;
    }

    // returns a copy of this DetElement but with the given weight as the residual weight
    DetElement withResidual(UnionSemiring.UnionWeight<GallicSemiring.GallicWeight> weight) {
